...
import anightdazingzoroark.rift.server.entity.creature.Triceratops;
...
public class RiftChargeAttack extends EntityAIBase {
    ...
    public void updateTask() {
        if (!this.attacker.world.isRemote) {
            if (this.attacker.chargeCooldown == 0) {
                ...
                if (this.attacker.isCharging()) {
                    ...
                    if (breakBlocksFlag || !chargedIntoEntities.isEmpty() || this.atSpotToChargeTo()) {
                        for (EntityLivingBase entity : chargedIntoEntities) this.attacker.attackEntityAsMob(entity);

                        List<BlockPos> toBreak = new ArrayList<>();
                        for (int x = MathHelper.floor(chargerHitbox.minX); x < MathHelper.ceil(chargerHitbox.maxX); x++) {
                            for (int y = MathHelper.floor(chargerHitbox.minY); x < MathHelper.ceil(chargerHitbox.maxY); y++) {
    ...
    protected boolean atSpotToChargeTo() {
        ...
        return distanceX <= 1D && distanceZ <= 1D;
    }
