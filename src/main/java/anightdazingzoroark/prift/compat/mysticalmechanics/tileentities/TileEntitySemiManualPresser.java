                ...
                if (this.getTopTEntity().getPower() > 0) {
                    if (this.getTopTEntity().getCurrentRecipe() == null) {
                        ...
                        if (!this.getTopTEntity().getMustBeReset() && !this.canDoResetAnim()) {
                            boolean outputUsability = this.getOutpuItem().isEmpty() || this.getOutpuItem().getCount() <= this.getOutpuItem().getMaxStackSize() || ((SemiManualPresserRecipe)this.getTopTEntity().getCurrentRecipe()).output.apply(this.getOutpuItem());
                            if (outputUsability) {
