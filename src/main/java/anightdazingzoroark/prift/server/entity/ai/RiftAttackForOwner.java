        ...
        if (!this.creature.isTamed()) return false;
            ...
            if (owner == null) return false;
                ...
                if (owner.getRidingEntity() instanceof RiftCreature) {
                    ...
                    return i != this.timestamp && this.isSuitableTarget(this.hitTarget, false) && this.creature.shouldAttackEntity(this.hitTarget, owner) &&  !this.creature.isBeingRidden() && !this.creature.isUsingWorkstation();
                }
            }
            else {
                this.hitTarget = owner.getLastAttackedEntity();
                int i = owner.getLastAttackedEntityTime();
                return i != this.timestamp && this.isSuitableTarget(this.hitTarget, false) && this.creature.shouldAttackEntity(this.hitTarget, owner) && !this.creature.isUsingWorkstation();
            }
        }
        return false;
    }
