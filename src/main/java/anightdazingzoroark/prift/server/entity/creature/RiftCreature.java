...
public abstract class RiftCreature extends EntityTameable implements IAnimatable, IMultiHitboxUser, IDynamicRideUser, IGuiHolder<CreatureGuiData> {
    ...
    public RiftCreature(World worldIn, RiftCreatureType creatureType) {
        ...
        this.creatureInventory = new CreatureInventoryHandler(creatureType.inventorySlots);
    ...
    protected void entityInit() {
        ...
        this.dataManager.register(DEPLOYMENT_TYPE, (byte) CreatureDeployment.NONE.ordinal());
    ...
    public void onLivingUpdate() {
        ...
        if (!this.world.isRemote) {
            ...
            if (this.isTamed()) {
                ...
                if (!this.hasTarget() && this.getDeploymentType() == CreatureDeployment.BASE && !this.creatureBoxWithinReach()) {
    ...
    private void eatFromInventory() {
        if (this.getHealth() < this.getMaxHealth()) {
            ...
            CreatureInventoryHandler.ItemSearchResult healFoodSearchResult = this.creatureInventory.findItem(
        ...
        if (this.getEnergy() < this.getMaxEnergy()) {
            ...
            CreatureInventoryHandler.ItemSearchResult energyFoodSearchResult = this.creatureInventory.findItem(
                    CreatureInventoryHandler.ItemSearchDirection.LAST_TO_FIRST,
    ...
    protected void manageSittingFromEnergy() {
        ...
        if (this.getEnergy() > this.getWeaknessEnergy()
            && this.isSitting()
            && this.getDeploymentType() == CreatureDeployment.BASE) this.setSitting(false);
    }
        ...
        if (this.isTamed()) {
            if (this.getOwner() != null) {
                if (this.isOwner(player) && !this.isPassenger(player)) {
                    ...
                    else if ((this.isTamingFood(itemstack) || itemstack.getItem() == RiftItems.CREATIVE_MEAL) && this.getHealth() >= this.getMaxHealth() && !this.isBaby() && this.getLoveCooldown() == 0 && !this.isSitting() && (playerPartyProperties.canAddToParty() || playerCreatureBoxProperties.canAddCreatureToBox())) {
                        this.consumeItemFromStack(player, itemstack);
            ...
            if (!itemstack.isEmpty() && (this.creatureType != RiftCreatureType.DODO) && (this.creatureType.isTameableByFeeding && this.isTamingFood(itemstack) || itemstack.getItem() == RiftItems.CREATIVE_MEAL) && (playerPartyProperties.canAddToParty() || playerCreatureBoxProperties.canAddCreatureToBox()) && !net.minecraftforge.event.ForgeEventFactory.onAnimalTame(this, player)) {
            ...
            else if (!itemstack.isEmpty() && (this.creatureType != RiftCreatureType.DODO) && (this.creatureType.isTameableByFeeding && this.isTamingFood(itemstack) || itemstack.getItem() == RiftItems.CREATIVE_MEAL) && !playerPartyProperties.canAddToParty() && !playerCreatureBoxProperties.canAddCreatureToBox() && !net.minecraftforge.event.ForgeEventFactory.onAnimalTame(this, player)) {
    ...
    public void tameCreature(EntityPlayer player) {
        ...
        if (!this.world.isRemote) {
            ...
            if (playerPartyProperties.canAddToParty()) {
                this.setDeploymentType(CreatureDeployment.PARTY);
            ...
            else if (playerCreatureBoxProperties.canAddCreatureToBox()) {
                this.setDeploymentType(CreatureDeployment.BASE_INACTIVE);
                playerCreatureBoxProperties.addCreatureToBox(this);
    ...
    public void writeEntityToNBT(@NotNull NBTTagCompound compound) {
        ...
        CreatureNBTKeyword.mergeResult(compound, CreatureNBTKeyword.LEARNED_MOVES, learnedMovesListNBT);

        //for learnable moves
        NBTTagCompound learnableMovesListNBT = MoveListUtil.getNBTFromListCreatureMove(this.getLearnableMoves());
        CreatureNBTKeyword.mergeResult(compound, CreatureNBTKeyword.LEARNABLE_MOVES, learnableMovesListNBT);
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        ...
        this.setDeploymentType(CreatureDeployment.values()[CreatureNBTKeyword.DEPLOYMENT_TYPE.parseValue(compound)]);
        this.setBoxReviveTime(compound.getInteger("BoxReviveTime"));

        //for learned moves
        NBTTagCompound learnedMovesListNBT = CreatureNBTKeyword.LEARNED_MOVES.parseValue(compound);
        ...
        //for learnable moves
        NBTTagCompound learnableMovesListNBT = CreatureNBTKeyword.LEARNABLE_MOVES.parseValue(compound);
    ...
    public void createBabyWhenPregnant() {
        if (this.getPregnancyTimer() > 0) {
            ...
            if (this.getPregnancyTimer() == 0) {
                ...
                PlayerCreatureBoxProperties playerCreatureBoxProperties = PlayerCreatureBoxHelper.getPlayerCreatureBox(owner);

                if (playerPartyProperties.canAddToParty()) {
                    baby.setDeploymentType(CreatureDeployment.PARTY);
                    ...
                    baby.setDeploymentType(CreatureDeployment.BASE_INACTIVE);
                    playerCreatureBoxProperties.addCreatureToBox(baby);
    ...
    public CreatureDeployment getDeploymentType() {
        return CreatureDeployment.values()[(int)this.dataManager.get(DEPLOYMENT_TYPE)];
    }

    public void setDeploymentType(CreatureDeployment value) {
    ...
    public void onDeath(DamageSource cause) {
        ...
        if (!this.world.isRemote && this.creatureInventory != null) {
            for (int i = 0; i < this.creatureInventory.getSlots(); i++) {
                ItemStack itemStack = this.creatureInventory.getStackInSlot(i);
                if (!itemStack.isEmpty()) {
                    this.entityDropItem(itemStack, 0.0f);
                    this.creatureInventory.setStackInSlot(i, new ItemStack(Items.AIR));
                }
            }
        }
    ...
    public void registerControllers(AnimationData data) {
        ...
        data.addAnimationController(new AnimationController(this, "movement", 0, new AnimationController.IAnimationPredicate() {
            ...
            public PlayState test(AnimationEvent event) {
                //for warning
                if (isWarning()) {
                    event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+creatureType.toString().toLowerCase()+".warn", LoopType.PLAY_ONCE));
                    return PlayState.CONTINUE;
                }
                //sitting
                else if (isSitting() && !isBeingRidden() && !hasTarget() && !isInWater()) {
                    event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+creatureType.toString().toLowerCase()+".sitting", LoopType.LOOP));
                    return PlayState.CONTINUE;
                }
                //swimming for water creatures
                else if (user instanceof RiftWaterCreature && user.isInWater()) {
                    event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+creatureType.toString().toLowerCase()+".swim", LoopType.LOOP));
                    return PlayState.CONTINUE;
                }
                //flopping for water creatures
                else if (user instanceof RiftWaterCreature && ((RiftWaterCreature)user).canFlop() && !user.isInWater()) {
                    event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+creatureType.toString().toLowerCase()+".flop", LoopType.LOOP));
                    return PlayState.CONTINUE;
                }
                //walking
        ...
        data.addAnimationController(new AnimationController(this, "sleep", 0, new AnimationController.IAnimationPredicate() {
            ...
            public PlayState test(AnimationEvent event) {
                if (isSleeping()) {
                    event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+creatureType.toString().toLowerCase()+".sleeping", LoopType.LOOP));
                    return PlayState.CONTINUE;
                }
                else if (isForcedAwake()) {
                    event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+creatureType.toString().toLowerCase()+".tired_pose", LoopType.LOOP));
                    return PlayState.CONTINUE;
                }
                event.getController().clearAnimationCache();
                return PlayState.STOP;
                    ...
                    if (getUnchargedAnimType() != null) {
                        if (currentCreatureMove().useTimeIsInfinite) {
                            if (getUnchargedMultistepMoveStep() > 0) {
                                LoopType animCanLoop = getUnchargedMultistepMoveStep() == 2 ? LoopType.LOOP : LoopType.PLAY_ONCE;
                        ...
                        else event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+creatureType.toString().toLowerCase()+".use_"+getUnchargedAnimType().toString().toLowerCase()+"_type_move"+multiNameNum, LoopType.PLAY_ONCE));
                        return PlayState.CONTINUE;
                    }
                    else {
                        chosenAnimFromMultiple = -1;
                ...
                if (currentCreatureMove() != null && currentCreatureMove().chargeType.requiresCharge()) {
                    if (movePos == 0) {
                        if (getPlayingChargedMoveAnim() == 0) {
                            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+creatureType.toString().toLowerCase()+".use_charged_"+currentCreatureMove().moveTypeName()+"_move_start", LoopType.PLAY_ONCE));
                        }
                        else if (getPlayingChargedMoveAnim() == 1) {
                            if (currentCreatureMove().useTimeIsInfinite) {
                                event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+creatureType.toString().toLowerCase()+".use_charged_"+currentCreatureMove().moveTypeName()+"_move_pt1", LoopType.LOOP));
                            }
                            else {
                                if (getMoveOneUse() > 0 && getMoveOneUse() < currentCreatureMove().maxUse) {
                                    event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+creatureType.toString().toLowerCase()+".use_charged_"+currentCreatureMove().moveTypeName()+"_move_pt1", LoopType.PLAY_ONCE));
                                }
                                else if (getMoveOneUse() >= currentCreatureMove().maxUse) {
                                    event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+ creatureType.toString().toLowerCase()+".use_charged_"+currentCreatureMove().moveTypeName()+"_move_pt1_hold", LoopType.LOOP));
                                }
                            }
                        }
                        else if (getPlayingChargedMoveAnim() == 2 || getPlayingChargedMoveAnim() == 3 || getPlayingChargedMoveAnim() == 4) {
                            if (currentCreatureMove().useTimeIsInfinite) {
                                if (getPlayingInfiniteMoveAnim()) event.getController().setAnimation(new AnimationBuilder().addAnimation("animation." + creatureType.toString().toLowerCase() + ".use_charged_infinite_" + currentCreatureMove().moveTypeName() + "_move_pt2", LoopType.LOOP));
                                else event.getController().setAnimation(new AnimationBuilder().addAnimation("animation." + creatureType.toString().toLowerCase() + ".use_charged_infinite_" + currentCreatureMove().moveTypeName() + "_move_pt3", LoopType.PLAY_ONCE));
                            }
                            else event.getController().setAnimation(new AnimationBuilder().addAnimation("animation." + creatureType.toString().toLowerCase() + ".use_charged_" + currentCreatureMove().moveTypeName() + "_move_pt2", LoopType.PLAY_ONCE));
                        }
                        ...
                        if (getPlayingChargedMoveAnim() == 0) {
                            ...
                            if (currentCreatureMove().useTimeIsInfinite) {
                                if (getPlayingChargedMoveAnim() == 3) {
                                    event.getController().setAnimation(new AnimationBuilder().addAnimation("animation." + creatureType.toString().toLowerCase() + ".use_charged_infinite_" + currentCreatureMove().moveTypeName() + "_move_pt2", LoopType.LOOP));
                                }
                                else if (getPlayingChargedMoveAnim() == 4) {
                                    event.getController().setAnimation(new AnimationBuilder().addAnimation("animation." + creatureType.toString().toLowerCase() + ".use_charged_infinite_" + currentCreatureMove().moveTypeName() + "_move_pt3", LoopType.PLAY_ONCE));
                                }
                            }
                            else event.getController().setAnimation(new AnimationBuilder().addAnimation("animation." + creatureType.toString().toLowerCase() + ".use_charged_" + currentCreatureMove().moveTypeName() + "_move_pt2", LoopType.PLAY_ONCE));
                        }
        ...
        data.addAnimationController(new AnimationController(this, "useLargeWeapon", 0, new AnimationController.IAnimationPredicate() {
            ...
            public PlayState test(AnimationEvent event) {
                ...
                if (getLargeWeapon() == RiftLargeWeaponType.CATAPULT) {
                    if (getUsingLargeWeapon() && canFireLargeWeapon()) event.getController().setAnimation(new AnimationBuilder().addAnimation("animation."+creatureType.toString().toLowerCase()+".charge_catapult", LoopType.LOOP));
                    else {
                        if (isFiringCatapult()) {
                            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation." + creatureType.toString().toLowerCase() + ".launch_catapult", LoopType.PLAY_ONCE));
                            return PlayState.CONTINUE;
                        }
                        else {
                            event.getController().clearAnimationCache();
