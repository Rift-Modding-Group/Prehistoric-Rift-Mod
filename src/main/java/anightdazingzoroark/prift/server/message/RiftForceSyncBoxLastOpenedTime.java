...
public class RiftForceSyncBoxLastOpenedTime implements IMessage {
    ...
    public static class Handler implements IMessageHandler<RiftForceSyncBoxLastOpenedTime, IMessage> {
        @Override
        public IMessage onMessage(RiftForceSyncBoxLastOpenedTime message, MessageContext ctx) {
            FMLCommonHandler.instance().getWorldThread(ctx.netHandler).addScheduledTask(() -> handle(message, ctx));
            return null;
        }

        private void handle(RiftForceSyncBoxLastOpenedTime message, MessageContext ctx) {
