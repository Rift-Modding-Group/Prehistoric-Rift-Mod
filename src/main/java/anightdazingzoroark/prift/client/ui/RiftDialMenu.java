...
public class RiftDialMenu extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i = 0; i < numItems; i++) {
            ...
            if (this.selectedItem == i) {
            ...
            else if (this.radialChoiceMenu == 3 && (!this.creature.canEnterTurretMode() || !this.creature.isTurretMode()) && this.creature.getTameBehavior().name().equals(this.choices.get(i).name())) {
                drawPieArc(buffer, x, y, zLevel, radiusIn, radiusOut, s, e, 128, 0, 128, 128);
            }
            else if (this.radialChoiceMenu == 3 && this.creature.canEnterTurretMode() && this.creature.isTurretMode() && this.creature.getTurretTargeting().name().equals(this.choices.get(i).name())) {
                drawPieArc(buffer, x, y, zLevel, radiusIn, radiusOut, s, e, 128, 0, 128, 128);
            }
            else if (this.radialChoiceMenu == 0 && (this.creature.isBaby() || (this.creature.canEnterTurretMode() && this.creature.isTurretMode()) || this.creature.busyAtWork() || this.creature.isSleeping()) && this.choices.get(i) == RiftTameRadialChoice.RIDE) {
            ...
            else if (this.radialChoiceMenu == 2 && this.creature.isBaby() && onlySelected && (this.choices.get(i) == RiftTameRadialChoice.SET_WORKSTATION || this.choices.get(i) == RiftTameRadialChoice.SET_WANDER_HARVEST)) {
                drawPieArc(buffer, x, y, zLevel, radiusIn, radiusOut, s, e, 0, 0, 0, 64);
            }
            else if (this.radialChoiceMenu == 2 && onlySelected && this.choices.get(i) == RiftTameRadialChoice.SET_TURRET_MODE && this.creature.busyAtWork()) {
                drawPieArc(buffer, x, y, zLevel, radiusIn, radiusOut, s, e, 0, 0, 0, 64);
            }
                ...
                if (this.creature instanceof ILeadWorkstationUser) {
                    ...
                    if (user.isAttachableForWork(((ILeadWorkstationUser)this.creature).getLeadWorkPos())) {
                        drawPieArc(buffer, x, y, zLevel, radiusIn, radiusOut, s, e, 0, 0, 0, 64);
                    }
                    else drawPieArc(buffer, x, y, zLevel, radiusIn, radiusOut, s, e, 0, 0, 0, 128);
                }
            ...
            else if (this.radialChoiceMenu == 0 && this.creature.busyAtWork() && onlySelected && (this.choices.get(i) == RiftTameRadialChoice.STATE || this.choices.get(i) == RiftTameRadialChoice.BEHAVIOR)) drawPieArc(buffer, x, y, zLevel, radiusIn, radiusOut, s, e, 0, 0, 0, 64);
            else if (this.radialChoiceMenu == 2 && this.creature.busyAtWork() && onlySelected && (this.choices.get(i) != RiftTameRadialChoice.BACK && this.choices.get(i) != RiftTameRadialChoice.SET_WORKSTATION)) drawPieArc(buffer, x, y, zLevel, radiusIn, radiusOut, s, e, 0, 0, 0, 64);
            else drawPieArc(buffer, x, y, zLevel, radiusIn, radiusOut, s, e, 0, 0, 0, 128);
        }
        ...
        for (int i = 0; i < numItems; i++) {
            ...
            if (this.radialChoiceMenu == 0 && this.choices.get(i) == RiftTameRadialChoice.BEHAVIOR) {
                if (this.creature.canEnterTurretMode() && this.creature.isTurretMode()) radialString = I18n.format("radial.choice.turret_targeting");
                ...
                if (!this.creature.busyAtWork()) radialString = I18n.format("radial.choice.set_workstation");
            ...
            else if (this.radialChoiceMenu == 2 && this.choices.get(i) == RiftTameRadialChoice.SET_TURRET_MODE) {
                boolean isInTurretMode = this.creature.canEnterTurretMode() && this.creature.isTurretMode();
                if (!isInTurretMode) radialString = I18n.format("radial.choice.set_turret_mode");
                else radialString = I18n.format("radial.choice.clear_turret_mode");
            }
            ...
            if (this.radialChoiceMenu == 0 && this.creature.isRideable && this.choices.get(i) == RiftTameRadialChoice.RIDE && (this.creature.isBaby() || (!this.creature.isBaby() && !this.creature.isSaddled()) || (this.creature.canEnterTurretMode() && this.creature.isTurretMode()) || this.creature.busyAtWork() || this.creature.isSleeping())) radialString = "["+radialString+"]";
            ...
            if (this.radialChoiceMenu == 2 && (this.choices.get(i) == RiftTameRadialChoice.SET_WORKSTATION || this.choices.get(i) == RiftTameRadialChoice.SET_WANDER_HARVEST) && this.creature.isBaby()) radialString = "["+radialString+"]";
            if (this.radialChoiceMenu == 2 && this.choices.get(i) == RiftTameRadialChoice.SET_TURRET_MODE && this.creature.busyAtWork()) radialString = "["+radialString+"]";
        ...
        if (this.selectedItem > -1 && this.selectedItem < numItems) {
            ...
            if (this.radialChoiceMenu == 0 && (this.creature.busyAtWork() || (this.creature.canEnterTurretMode() && this.creature.isTurretMode())) && this.choices.get(this.selectedItem) == RiftTameRadialChoice.RIDE) {
            ...
            if (this.radialChoiceMenu == 2 && this.creature.busyAtWork() && this.choices.get(this.selectedItem) == RiftTameRadialChoice.SET_TURRET_MODE) {
                this.drawHoveringText(I18n.format("radial.note.too_busy"), mouseX, mouseY);
            }
            if (this.radialChoiceMenu == 0 && this.creature.busyAtWork() && this.choices.get(this.selectedItem) == RiftTameRadialChoice.STATE) {
                this.drawHoveringText(I18n.format("radial.note.too_busy"), mouseX, mouseY);
            }
            if (this.radialChoiceMenu == 0 && this.creature.busyAtWork() && this.choices.get(this.selectedItem) == RiftTameRadialChoice.BEHAVIOR) {
                this.drawHoveringText(I18n.format("radial.note.too_busy"), mouseX, mouseY);
            }
            ...
            case 0:
                if (this.selectedItem > -1 && this.selectedItem < this.creature.mainRadialChoices().size()) {
                    if (this.creature.mainRadialChoices().get(this.selectedItem) == RiftTameRadialChoice.INVENTORY) {
                        ...
                        boolean isNotWorkedLead = !(this.creature instanceof ILeadWorkstationUser) || !((ILeadWorkstationUser) this.creature).isAttachableForWork(((ILeadWorkstationUser)this.creature).getLeadWorkPos());
                        if (isNotWorkedLead) {
            ...
            case 2:
                if (this.selectedItem > -1 && this.selectedItem < this.creature.optionsRadialChoices().size()) {
                    if (this.creature.optionsRadialChoices().get(this.selectedItem) == RiftTameRadialChoice.BACK) {
                        ...
                        RiftMessages.WRAPPER.sendToServer(new RiftSetTurretMode(this.creature, !this.creature.isTurretMode()));
                        this.mc.player.closeScreen();
                    }
                }
                break;
                ...
                if (isInTurretMode) {
                    if (this.selectedItem > -1 && this.selectedItem < this.creature.turretRadialChoices().size()) {
                        if (this.creature.turretRadialChoices().get(this.selectedItem) == RiftTameRadialChoice.BACK) {
                            ...
                            RiftMessages.WRAPPER.sendToServer(new RiftChangeTurretTargetingFromMenu(this.creature, TurretModeTargeting.PLAYERS));
                            this.mc.player.closeScreen();
                        }
                        else if (this.creature.turretRadialChoices().get(this.selectedItem) == RiftTameRadialChoice.PLAYERS_AND_OTHER_TAMES) {
                            RiftMessages.WRAPPER.sendToServer(new RiftChangeTurretTargetingFromMenu(this.creature, TurretModeTargeting.PLAYERS_AND_OTHER_TAMES));
                            this.mc.player.closeScreen();
                        }
                        else if (this.creature.turretRadialChoices().get(this.selectedItem) == RiftTameRadialChoice.HOSTILES) {
                            RiftMessages.WRAPPER.sendToServer(new RiftChangeTurretTargetingFromMenu(this.creature, TurretModeTargeting.HOSTILES));
                            this.mc.player.closeScreen();
                        }
                        else if (this.creature.turretRadialChoices().get(this.selectedItem) == RiftTameRadialChoice.ALL) {
                            RiftMessages.WRAPPER.sendToServer(new RiftChangeTurretTargetingFromMenu(this.creature, TurretModeTargeting.ALL));
                            this.mc.player.closeScreen();
                        }
                    }
                }
