...
public class PaddedGrid extends AbstractScrollWidget<IWidget, PaddedGrid> implements ILayoutWidget, IParentWidget<IWidget, PaddedGrid> {
    ...
    public boolean layoutWidgets() {
        ...
        for (int r = 0; r < rowSizes.size(); r++) {
            ...
            for (int c = 0; c < colSizes.size(); c++) {
                ...
                //add padding between columns except for last
                if (c < colSizes.size() - 1) x += this.padding;
            }
            y += height;
            if (r < rowSizes.size() - 1) y += this.padding;
        }
    ...
    public <I extends IWidget> PaddedGrid matrix(List<List<I>> matrix) {
        this.matrix.clear();
    ...
    public PaddedGrid row(List<IWidget> row) {
        this.matrix.add(row);
        this.dirty = true;
        return this;
    }

    public PaddedGrid row(@NotNull IWidget... row) {
    ...
    public PaddedGrid child(@Nullable IWidget widget) {
        this.matrix.get(this.matrix.size() - 1).add(widget);
        this.dirty = true;
        return this;
    }

    public PaddedGrid nextRow() {
        this.matrix.add(new ArrayList<>());
        return this;
    }

    public <T, I extends IWidget> PaddedGrid mapTo(int rowLength, @NotNull List<T> list, @NotNull Grid.IndexedElementMapper<T, I> widgetCreator) {
        Objects.requireNonNull(widgetCreator);
        Objects.requireNonNull(list);
        return matrix(mapToMatrix(rowLength, list, widgetCreator));
    }

    public <I extends IWidget> PaddedGrid mapTo(int rowLength, @NotNull List<I> list) {
        Objects.requireNonNull(list);
        return mapTo(rowLength, list.size(), list::get);
    }

    public <I extends IWidget> PaddedGrid mapTo(int rowLength, int size, @NotNull IntFunction<I> widgetCreator) {
        Objects.requireNonNull(widgetCreator);
        return matrix(mapToMatrix(rowLength, size, widgetCreator));
    }

    public PaddedGrid minColWidth(int minColWidth) {
        this.minColWidth = minColWidth;
        return this;
    }

    public PaddedGrid minRowHeight(int minRowHeight) {
        this.minRowHeight = minRowHeight;
        return this;
    }

    public PaddedGrid alignment(Alignment alignment) {
        this.alignment = alignment;
        return this;
    }

    public PaddedGrid scrollable() {
        return scrollable(new VerticalScrollData(), new HorizontalScrollData());
    }

    public PaddedGrid scrollable(ScrollData data) {
        getScrollArea().setScrollData(data);
        return this;
    }

    public PaddedGrid scrollable(VerticalScrollData data1, HorizontalScrollData data2) {
        getScrollArea().setScrollData(data1);
        getScrollArea().setScrollData(data2);
        return this;
    }

    public PaddedGrid minElementMargin(int left, int right, int top, int bottom) {
        this.minElementMargin.all(left, right, top, bottom);
        return getThis();
    }

    public PaddedGrid minElementMargin(int horizontal, int vertical) {
        this.minElementMargin.all(horizontal, vertical);
        return getThis();
    }

    public PaddedGrid minElementMargin(int all) {
        this.minElementMargin.all(all);
        return getThis();
    }

    public PaddedGrid minElementMarginLeft(int val) {
        this.minElementMargin.left(val);
        return getThis();
    }

    public PaddedGrid minElementMarginRight(int val) {
        this.minElementMargin.right(val);
        return getThis();
    }

    public PaddedGrid minElementMarginTop(int val) {
        this.minElementMargin.top(val);
        return getThis();
    }

    public PaddedGrid minElementMarginBottom(int val) {
        this.minElementMargin.bottom(val);
        return getThis();
    }

    public PaddedGrid padding(int val) {
        this.padding = val;
        return getThis();
    }
     ...
     */
    public PaddedGrid collapseDisabledChild() {
        this.collapseDisabledChild = true;
        return getThis();
    }

    public static <T, I extends IWidget> List<List<I>> mapToMatrix(int rowLength, List<T> list, Grid.IndexedElementMapper<T, I> widgetCreator) {
